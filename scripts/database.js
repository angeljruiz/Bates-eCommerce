...
class Database {
  static className(model) {
    return new model().constructor.name + " as lh ";
  }
  static publicKey(pk) {
    if (pk != "all" && pk)
      return (
        "WHERE lh." + pk[0] + " = '" + pk[1] + "'" + (pk[2] ? " " + pk[2] : "")
      );
    else return "";
  }
  static joinNeeded(model, attrs, pk) {
    ...
    if (
      ...
      Object.getPrototypeOf(model).name == "Persistent"
    )
      return false;
    let keys = model.variables();
    if (attrs == "all") return true;
    attrs.forEach((item) => {
      if (!keys.includes(item)) needed = true;
    });
    return needed;
  }
  ...
  static attributes(attrs, pre = "", encap = "", post = "") {
    ...
    if (attrs == "all") {
      ...
      attrs.forEach((item, index) => {
        ...
        if (index < attrs.length - 1) attributes += ", ";
      });
    }
  ...
  saveData(model, attrs, pk, input) {
    return new Promise((resolve, reject) => {
      ...
      query +=
        Database.className(model) + Database.attributes(attrs, pre, "", post);
      query += pk ? " = (" : " VALUES (";
      query +=
  ...
  deleteData(model, pk, fn) {
    return new Promise((resolve, reject) => {
      let query =
        "DELETE FROM " + Database.className(model) + Database.publicKey(pk);
      console.log(query);
      pool.query(query, (err) => {
        if (err) return reject(err);
        else return resolve(true);
